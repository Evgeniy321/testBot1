...
total_score = 0
substory = [[
    "Привет! Этот бот является связью между мирами, в которых такая же Аня и Женя встречают новый год в двоем и думают что им поделать...",
...
quest = [
  ...
  #qusetion_count = 6
  [["**Переход на последний квест**", "**Время и локация: сейчас**","Слушай, кажется к нам пришел дед мороз, надо его как то встретить по скорее"],["Похоже на то, как иронично)", "Давай отогреем его, чем то что не сильно жалко, но и просто кленкой для бомжей настилать не будем"],["Все, он согрелся, но видимо он шел не к нам","Есть тот, кто жил с нами достаточно долго, но мы про него забыли, и он подарил ему подарок"],["Да, все верно))\nЖоре очень понравился твой подарок, но и ты сама что то получила на новый год от своего не вымышленого деда)))"],["Молодец, поздравляю тебя, ты выиграла)))", "Самый ценный подарок для меня это ты и в любом мире это останеться истиной, поздарвлю с новым годом и пусть он пройдет на много лучше вем предыдущий. В качесте подарка я предлагаю мгновенное обновление одного из купонов которые я подарил тебе ранее"],["сегодня было холодно даже для деда мороза, он выглядит не очень хорошо, что же с ним может быть?","Мы очень не давно купили одну прикольную и теплую штуку, наверное он не обидиться если мы дадим ее", "Такой маленький но не совсем, высокий, но не сам, а еще пушистый и его сородичи живут в ванной","То при помощи чего тебе даже не надо ничего кастовать, ты это увидишь в будующем и не через этот портал)"]]]
    ...
    if message.chat.id == chat_id:
        if "продолжить" in message.text.lower():
            ...
            if question_num > len(quest[story_counter]) - 3:
                ...
                if story_counter == 2:
                  bot.send_message(chat_id, f"Твой финальный счет: {total_score}")
            ...
            for i in range(len(list(answer[story_counter][question_num]))):
                try:
                    ...
                    break
            tip+="*"*(abs(len(answer[story_counter][question_num]) - len(message.text)))
